module.exports = {
	...
	has_state: false,
	options: [
		{ name: 'role_name', description: 'Name of the role to list members of', type: 'ROLE', required: true },
	],
	async execute(message, args, extra) {
		const { EmbedBuilder } = require('discord.js');
		role = message.guild.roles.cache.get(args[1] ? args[1].replace(/[<@&>]/g, '') : null); // Fast path if ID is provided
		if (!role) {
