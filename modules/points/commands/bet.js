        ...
        if(respCheckServer.bet_configs[0]){
            ...
            if(Number.isInteger(parseInt(args[1])) || args[1].charAt(args[1].length-1)==="%"){
                if(Number.isInteger(parseInt(args[1].slice(0,-1))) && args[1].charAt(args[1].length-1)==="%"){
                    ...
                    message.channel.send({content: "Created bet with " + respCheckBal.bet_points[0].points_total.toString() + " and percent: " + percent.toString()+" together that is: "+Math.floor(parseInt(respCheckBal.bet_points[0].points_total) * percent).toString()});
                }else {
